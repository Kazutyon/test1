...
document.getElementById("verify").addEventListener("click", () => {
  try {
    ...
    const sig = window.rsa.lastSignature;
    log(`✅ 検証: 署名 = ${sig}`);
    const mPrime = modPow(sig, window.rsa.e, window.rsa.N);
    const ok = mPrime === window.rsa.lastSignMsg;
    log(`検証結果 m' = ${mPrime}`);
    showFade(document.getElementById("verify-output"), ok ? "検証成功：署名は有効です" : "検証失敗：署名が一致しません");
    log(ok ? "✅ 検証成功" : "❌ 検証失敗");
  } catch (err) {
    alert(err.message);
  }
